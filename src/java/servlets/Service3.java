...
public class Service3 extends HttpServlet {
        ...
        HttpSession session = request.getSession(false);
         if (request.getParameter("translate") != null) {
            ...
            if(language.equals("kr")){
                response.sendRedirect("/kr/service3");
            }
            else{
                response.sendRedirect("/en/service3");
            }
            return;
        }
    ...
    if (logout != null) {
        ...
        response.sendRedirect("service3");
        return;
    }
    ...
    if (language == null) {
        ...
        if (language.equals("kr")) {
            getServletContext().getRequestDispatcher("/WEB-INF/kr/s3_massage.jsp").forward(request, response);
        } else if (language.equals("en")) {
            getServletContext().getRequestDispatcher("/WEB-INF/en/s3_massage.jsp").forward(request, response);
        }
    }
