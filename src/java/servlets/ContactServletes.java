 ...
 * @author Hussein
 */
public class ContactServletes extends HttpServlet {
        ...
        HttpSession session = request.getSession(false);
         if (request.getParameter("translate") != null) {
            ...
            if(language.equals("kr")){
                response.sendRedirect("/kr/contact");
            }
            else{
                response.sendRedirect("/en/contact");
            }
            return;
        }
    ...
    if (language == null) {
        ...
        if (language.equals("kr")) {
            getServletContext().getRequestDispatcher("/WEB-INF/kr/contact.jsp").forward(request, response);
